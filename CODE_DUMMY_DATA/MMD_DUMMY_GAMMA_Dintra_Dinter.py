...
def forward(model, classifier_layer_1, classifier_layer_2, classifier_layer_3, data, labels_source, labels_target, criterion, MMD_loss_calculator, GAMMA, BETA, ETA):
        ...
        # min D_inter max D_intra

        D_inter_1 = torch.sum(torch.abs(torch.var(out_src[labels_source==0], dim=0) - torch.var(out_src[labels_source==1], dim=0)) -torch.var(out_src[labels_source==0], dim=0) - torch.var(out_src[labels_source==1], dim=0))
        D_inter_2 = torch.sum(torch.abs(torch.var(x_src[labels_source==0], dim=0) - torch.var(x_src[labels_source==1], dim=0)) -torch.var(x_src[labels_source==0], dim=0) - torch.var(x_src[labels_source==1], dim=0))
        D_inter_3 = torch.sum(torch.abs(torch.var(source_out[labels_source==0], dim=0) - torch.var(source_out[labels_source==1], dim=0)) -torch.var(source_out[labels_source==0], dim=0) - torch.var(source_out[labels_source==1], dim=0))

        D_intra_1 = torch.sum(torch.var(out_src[labels_source==0], dim=0)) + torch.sum(torch.var(out_src[labels_source==1], dim=0))
        D_intra_2 = torch.sum(torch.var(x_src[labels_source==0], dim=0)) + torch.sum(torch.var(x_src[labels_source==1], dim=0))
        D_intra_3 = torch.sum(torch.var(source_out[labels_source==0], dim=0)) + torch.sum(torch.var(source_out[labels_source==1], dim=0))


        D_inter_intra = - D_inter_3 + ETA * D_intra_3
        ...
        #mmd_loss = mmd_loss_2
        #total loss
        loss = ce_loss + GAMMA * mmd_loss + BETA * D_inter_intra
...
if __name__ == "__main__":
    ...
    GAMMA = 0.4# 1000 more weight to transferability
    BETA = 0.25
    ETA = 1
        ...
        for phase in phases:
            ...
            for i in range(len(dataloaders[phase]["source"])):
                ...
                if phase == "val":
                    ...
                    with torch.no_grad():
                        loss, mmd_loss, source_ce_loss, target_ce_loss, acc_total_source, acc_total_target, class_0_source, class_1_source, class_0_target, class_1_target = forward(model, classifier_layer_1, classifier_layer_2, classifier_layer_3, data, labels_source, labels_target, criterion, MMD_loss_calculator, GAMMA, BETA, ETA)
                    ...
                    ########Forward pass########
                    loss, mmd_loss, source_ce_loss, target_ce_loss, acc_total_source, acc_total_target, _, _, _, _ = forward(model, classifier_layer_1, classifier_layer_2, classifier_layer_3, data, labels_source, labels_target, criterion, MMD_loss_calculator, GAMMA, BETA, ETA)
